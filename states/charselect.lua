...
function charselect:load()
	...
	self.cursors = {}
	for i = 1, 1 do
		self.cards[i] = card:new(i, 120, 200)
		self.cursors[i] = vector(1, 1)
	end
	self.cards[1]:setInfo(self.grid[1][1].character)
end
...
function charselect:update(dt)
	...
	for _, playerCard in ipairs(self.cards) do
		playerCard:update(dt)
	end
end
